...
namespace :workers do
  ...
  # = $ rake workers:start
  #
  # Launch multiple Resque workers with the Padrino environment loaded,
  ...
  task :start => :environment do
    ...
    def pid_directory
      @pid_directory ||= Padrino.root('tmp', 'pids', "resque")
    end

    def pid_directory_for_group
      @pid_directory_for_group ||= Padrino.root('tmp', 'pids', "resque", queue)
    end

    def group_master_pid
      File.read File.join(pid_directory, "#{queue}.pid") rescue nil
    end
    ...
    def kill_workers_and_remove_pids_for_group
      Dir.glob(File.join(pid_directory_for_group, 'worker_*.pid')) do |pidfile|
        begin
      ...
      if group_master_pid
        FileUtils.rm  File.join(pid_directory, "#{queue}.pid")
        FileUtils.rm_rf pid_directory_for_group
      end
    end
    ...
    if group?
      ...
      FileUtils.mkdir_p pid_directory_for_group
      # Create PID files for workers
      File.open( File.join(pid_directory, "#{queue}.pid"), 'w' ) do |f| f.write Process.pid end
      @pids.each do |pid|
        File.open( File.join(pid_directory_for_group, "worker_#{pid}.pid"), 'w' ) { |f| f.write pid }
      end
