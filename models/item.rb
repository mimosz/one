...
class Item # 商品
  ...
  # 从上架日期起，近7天的
  field :duration,        type: Integer,   default: 0  # 周转天数
  ...
  class << self
    ...
    def sync_create(session, status = :onsale, page_no = 1, page_size = 200)
      ...
      options = { session: session, method: method, page_no: page_no, page_size: page_size, fields: item_fields }
      process_sync(options)
    end
    
    def sync_update(user, start_at = 7.day.ago.to_date, end_at = 1.day.ago.to_date)
      items = user.items
      items_res = {}
      skus_res = {}
      unknown_items = [] # 未知产品
      ...
      items.each do |item|
        if items_res.has_key?(item.num_iid)
          ...
          item_res[:duration] = item_res[:range].count
      ...
      if unknown_items.count > 0
        puts "Item.create_items============================补失"
        create_items(user.session, unknown_items)
      end
    end

    private

    def create_items(session, item_ids, limit = 20)
      result = []
      ...
      Sku.sync_create(session, result)
    end
