...
module Taobao
  module Sync
    ...
    def subusers_sync
      Taobao::Subuser.sync_create(session, nick)
    end
      ...
      case 
        when duration >= limit # 多日
          ...
          range.each_slice(limit).each do | day |
            day = day.last
            Taobao::Chatpeer.sync_create(session, users, day.beginning_of_day, day.end_of_day)
          end
        when duration == 0 # 单日
         Taobao::Chatpeer.sync_create(session, users, start_at.beginning_of_day, end_at.end_of_day)
        else
    ...
    def wangwangs_sync(start_at = nil, end_at = Date.yesterday, limit = 1, expire_at = 7.day.ago.to_date)
      ...
      case 
        when duration >= limit # 多日
          ...
          range.each_slice(limit).each do | day |
            day = day.last
            Taobao::Wangwang.sync_create(self, day.beginning_of_day, day.end_of_day)
          end
        when duration == 0 # 单日
         Taobao::Wangwang.sync_create(self, start_at.beginning_of_day, end_at.end_of_day)
       else
         puts "从#{start_at}到#{end_at}。"
      end
    end
    ...
    def trades_sync(start_at = nil, end_at = Date.today, limit = 1, expire_at = 90.day.ago.to_date) # 交易
      ...
      case 
        when duration >= limit # 多日
          Taobao::Trade.sync_create(session, start_at.beginning_of_day, end_at.end_of_day)
        when duration == 0 # 单日
          Taobao::Trade.sync_update(session, start_at.beginning_of_day, end_at.end_of_day)
        else
         puts "从#{start_at}到#{end_at}。"
      end
    end
    ...
    def shippings_sync
      ...
      if send.exists?
        ...
        Taobao::Shipping.sync_create(session, start_at, Time.now)
      end
    end
    
    def orders_sync
      Taobao::Trade.sync_orders(session, trades.sync_ids)
      Taobao::Item.sync_update(self)
    end

    def items_sync # 商品
      Taobao::Item.sync_create(session, :onsale)
      Taobao::Item.sync_create(session, :inventory)
      Taobao::Item.sync_items(session, items.sync_ids)
    end
    
    def addresses_sync # 卖家地址
      Taobao::Address.sync_create(self)
    end
    ...
    def members_sync(start_at=nil, end_at=Date.today, grade=nil) # 卖家的会员
      if members.exists?
        ...
        Taobao::Member.sync_update(self, start_at.beginning_of_day, end_at.end_of_day, grade)
      else
        Taobao::Member.sync_create(self)
      end
    end
    ...
    def refunds_sync # 退款
      ...
      Taobao::Refund.sync_create(session, start_at, Time.now)
    end
    
    def rates_sync # 评价，每日更新
      Taobao::Rate.sync_create(self)
    end
