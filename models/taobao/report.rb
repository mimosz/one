    ...
    def report_by_sales(batch, order)
      report = case
        when order.is_oversold
          batch[:oversold]   = Taobao::OversoldDaily.new  unless batch.has_key?(:oversold) # 超卖
          batch[:oversold]
        when trade.is_brand_sale
          batch[:brand_sale] = Taobao::BrandSaleDaily.new unless batch.has_key?(:brand_sale) # 特卖
          batch[:brand_sale]
        else
          batch[:normal]     = Taobao::Daily.new          unless batch.has_key?(:normal) # 常规
          batch[:normal]
        end
