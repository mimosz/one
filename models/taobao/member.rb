...
class Taobao::Member 
  ...
  def cache_receivers
    if synced_at.nil?
      trade = Taobao::Trade.where(_id: biz_order_id).last
      if trade
        ...
        if current_receiver.nil?
          ...
          self.receivers << Taobao::Receiver.new( trade.merge(mobile_carrier: specs[:carrier], mobile_network: specs[:network]) )
        end
        self.save
      end
    end
  ...
  def trades # 交易
    ...
    Taobao::Trade.where( conditions ).desc(:created, :modified)
  end
  
  def chatpeers # 聊天记录
    conditions = { seller_nick: seller_nick, uid: "cntaobao#{buyer_nick}" }
    Taobao::Chatpeer.where( conditions ).desc(:date)
  end

  def refunds # 退款
    conditions = { seller_nick: seller_nick, buyer_nick: buyer_nick }
    Taobao::Refund.where( conditions ).desc(:created, :modified)
  end
